...
property_given(Name, Props) :- property(Name, Props), !.
property_given(Name, L) :- group(Name), size(Name, N), findall(Num, between(1, N, Num), L) .
...
prop2num(Name, Prop, N) :- 
    property_given(Name, Props),
    nth1(N, Props, Prop), !.
prop2num(Name, Prop, Prop) :-
    \+property_given(Name,_).
