% M434: Two sets of cards with a letter on each card as follows are placed into separate bags.  The fist bag contains letters I, L, J, A, U, the second bag contains letters L, R, H, E, C, A. Sara randomly picked one card from each bag.  What is the probability that both letters are L? ## Solution= 1/30
