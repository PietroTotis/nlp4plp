...
from problog_util import ProblogDataset
...
class Nlp4plpInst:
    def __init__(self, ls, p_data, low=True, tokenizer=word_tokenize):
        self.id, self.txt, self.ans, self.statements = self.read(ls, low, tokenizer, p_data)

    @staticmethod
    def read(ls, low, tokenize, p_data):
        ...
        ans = p_data.label[problem_id]
        return problem_id, problem_txt_lst, ans, statements


class Nlp4plpCorpus:
    def __init__(self, data_dir, label_type):
        self.data_dir = data_dir
        self.labels = ProblogDataset(data_dir, label_type)
    ...
    def get_insts(self):
        ...
        insts = []
        for f in dir_fs:
            inst = Nlp4plpInst(Nlp4plpData.read_pl(f), self.labels)
